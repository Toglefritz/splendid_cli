...
/// Controller for the {{name.lowerCase()}} screen that manages state and
/// business logic.
///
/// Extends `State<{{name.pascalCase()}}Route>` to provide state management
/// capabilities and serves as the bridge between the route and view components.
/// Manages the icon selection game logic including randomization and user
/// interactions.
class {{name.pascalCase()}}Controller extends State<{{name.pascalCase()}}Route> {
  /// Available icons for the selection game.
  ///
  /// Contains the three Material icons that users can select from: rocket, chef
  /// hat, and art palette.
  ...
  ///
  /// This list is shuffled each time the game resets to provide variety in icon
  /// positioning.
  ...
  ///
  /// This method is called during initialization and after each successful
  /// selection to provide a new challenge for the user.
  void _resetGame() {
    setState(() {
  ...
  ///
  /// When the user selects the correct icon, the game resets with a new
  /// arrangement. Incorrect selections have no effect, allowing the user to try
  /// again.
  ///
  /// Parameters:
