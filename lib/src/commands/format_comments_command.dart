...
///
/// This command provides automated reformatting of regular // comments to
/// adjust line lengths and improve consistency across a codebase. It's
/// particularly useful for migrating from 80-character to 120-character line
/// limits or standardizing comment formatting across team projects.
///
/// The command operates on individual files or entire directory trees,
/// intelligently processing only Dart files while preserving code structure and
/// special formatting elements like code blocks, lists, and headers. It
/// specifically targets regular // comments and excludes /// Dartdoc comments.
///
/// Key Features:
...
///
/// Performance: Optimized for large codebases with efficient file I/O and
/// minimal memory usage. Typical processing speed is 50-100 files per second
/// depending on file size and comment density.
///
...
class FormatCommentsCommand extends Command<int> {
  /// Creates a new instance of [FormatCommentsCommand] with configured argument
  /// parser.
  ///
  ...
  @override
  String get invocation => 'splendid_cli format-comments <target> [line-length] [arguments]';

  /// Executes the comment formatting command with parsed command-line
  /// arguments.
  ///
  ...
  Future<int> run() async {
    ...
    if (argResults!.rest.isEmpty) {
      logger
        ...
        ..info('       splendid_cli format-comments <target> <line-length>')
        ..info('')
        ..info('Examples:')
        ..info('  splendid_cli format-comments .')
        ..info('  splendid_cli format-comments lib/services/api_service.dart 80')
    ...
    try {
      ...
      ///
      /// Encapsulates the business logic for regular comment processing and
      /// provides a clean interface for the command layer.
      const CommentFormatterService service = CommentFormatterService();

      /// Request configuration for the formatting operation.
      ///
      /// Contains all parameters needed to execute the formatting process
