...
///
/// This command creates boilerplate test files for Dart classes and Flutter widgets,
/// following established testing patterns and documentation standards. It uses Mason
/// bricks to ensure consistent test structure across the project.
///
/// The command analyzes the target Dart file to determine whether it contains a
/// Flutter widget or regular Dart class, then generates the appropriate test template:
...
class TestCommand extends Command<int> {
  ...
  Future<int> run() async {
    ...
    ///
    /// Must be a valid Dart file (.dart extension) that exists in the file system.
    /// The file will be analyzed to determine the appropriate test template type.
    final String targetFile = argResults!.rest.first;

    /// Optional custom output directory specified via --output flag.
    ///
    /// When provided, the test file will be created in this directory.
    /// When null, the test file location is determined by mirroring the
    ...
    /// When false, the command will fail if the target test file already exists.
    ...
    try {
      ...
      ///
      /// Contains the class name, file name, and other metadata needed
      /// to generate appropriate test templates.
      final FileAnalysis analysis = await _analyzeFile(sourceFile, logger);

      /// Absolute path where the test file will be created.
      ///
      /// Determined by either the explicit output directory or by mirroring
      /// the source file structure in the test/ directory.
      ...
      ///
      /// The generator contains template files for either widget tests or class tests
      /// based on the resolved test type.
  ...
  ///
  /// The extracted information is used to populate template variables
  /// for generating appropriate test files.
  ///
  /// Parameters:
  /// * [file] - The Dart file to analyze
  /// * [logger] - Logger instance for debug output
  ...
  /// file structure in the test/ directory, unless a custom output directory
  /// is specified.
  ///
  ...
  /// specified test type (widget or class) from the CLI package's brick directory.
  ///
  /// Brick resolution:
  ...
  Future<MasonGenerator> _loadTestBrick(String testType, Logger logger) async {
    try {
      ...
      ///
      /// Maps test types to their corresponding brick names in the
      ...
      ///
      /// The brick contains metadata (brick.yaml) and template files
      /// (__brick__ directory) that define the structure and content
  ...
  ///
  /// This method constructs the variable map that will be passed to the
  /// Mason brick template, containing all necessary information for
  /// generating a complete and properly structured test file.
  ///
  ...
  Map<String, dynamic> _buildTemplateVariables(
    ...
    ///
    /// Calculated to ensure the test file can properly import the
    /// class being tested, following Dart import conventions.
    final String importPath = _calculateImportPath(sourceFile);

    /// CamelCase version of the file name for variable naming.
    ///
    /// Converts snake_case file names to camelCase for proper
    /// Dart variable naming conventions.
  ...
  ///
  /// This method determines the correct import statement that should be
  /// used in the generated test file to import the class being tested.
  ///
  ...
  ///
  /// This method reads the pubspec.yaml file to extract the package name
  /// that should be used in import statements within generated test files.
  ///
  /// Returns:
  ...
  ///
  /// This utility method is used to generate appropriate class names
  /// when the primary class name cannot be extracted from the source file.
  ///
  /// Conversion rules:
...
///
/// This class contains metadata extracted from analyzing a Dart file,
/// used for generating appropriate test templates and import statements.
///
...
class FileAnalysis {
  ...
  ///
  /// All parameters are required as they provide essential information
  /// for test generation and template variable construction.
  const FileAnalysis({
  ...
  ///
  /// Used for generating test class names and documentation.
  /// Extracted from the first public class declaration found in the file.
  final String className;

  /// The base file name without extension.
  ///
  /// Used for generating test file names and import statements.
  /// Derived from the file path by removing directory and extension.
  final String fileName;

  /// The full path to the analyzed file.
  ///
  /// Used for calculating relative import paths and determining
  /// the appropriate test file location.
  final String filePath;

  /// Whether the file contains Flutter framework imports.
  ///
  /// Used to determine if Flutter-specific testing utilities
  /// should be included in the generated test template.
