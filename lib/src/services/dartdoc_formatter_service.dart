...
/// files or entire directory trees while maintaining the original file structure.
///
...
///
/// Performance: Processing is optimized for large codebases with minimal
/// memory usage and efficient file I/O operations.
class DartdocFormatterService {
  ...
  /// reformats all Dartdoc comments according to the specified line length.
  /// It handles both single files and recursive directory processing.
  ///
  /// Processing workflow:
  ...
  ///
  /// Ensures that all required parameters are valid and the target
  /// path exists and is accessible for processing.
  ...
  /// recursively searches for all .dart files while respecting common
  /// ignore patterns (build/, .dart_tool/, etc.).
  ...
  ///
  /// Skips files in common build and tool directories that typically
  /// contain generated code or temporary files.
  ...
  ///
  /// Reads the file content, identifies and reformats Dartdoc comments,
  /// and writes the updated content back to the file (unless in dry-run mode).
  ///
  ...
  ///
  /// This method processes the entire file content and reformats all
  /// Dartdoc comments according to the specified line length while
  /// preserving code structure and non-comment content.
  ...
  ///
  /// Recognizes both /// and /** comment styles, accounting for
  ...
  ///
  /// Identifies the full extent of a Dartdoc comment, handling both
  ...
  ///
  /// Handles the wrapping and reformatting of triple-slash comments
  /// while maintaining proper indentation and comment syntax.
  ...
  ///
  /// Performs intelligent word wrapping that respects word boundaries
  /// and maintains readability while fitting within the specified length.
...
/// including its content, formatting style, and positioning information
/// needed for proper reformatting.
class DartdocBlock {
  ...
  ///
  /// Includes the original comment syntax and formatting, which will
  /// be processed during reformatting operations.
  ...
  ///
  /// Maintains the original indentation level of the comment block
  /// to preserve code structure and formatting.
  final String leadingWhitespace;

  /// Whether this is a multi-line (/** */) or single-line (///) comment.
  ///
  /// Determines which formatting rules and syntax should be applied
...
///
/// This class encapsulates all parameters needed to configure the
/// formatting process, including target specification, line length
...
class DartdocFormatterRequest {
  ...
  ///
  /// Can be either a single .dart file or a directory containing
  /// Dart files to process recursively.
  final String targetPath;

  /// Maximum line length for wrapped comment text.
  ///
  /// Comments will be reformatted to fit within this character limit
  /// while respecting word boundaries and special formatting.
  final int lineLength;

  /// Whether to perform a dry run without modifying files.
  ///
  /// When true, the service will analyze and report what changes
  /// would be made without actually writing to files.
  final bool dryRun;
}

/// Result of a Dartdoc formatting operation.
///
/// This class contains comprehensive information about the formatting
/// process, including statistics, file lists, and any errors encountered
/// during processing.
class DartdocFormatterResult {
  ...
  ///
  /// True if all files were processed without critical errors,
  /// false if the operation failed or was aborted.
  final bool success;

  /// List of all files that were processed.
  ///
  /// Includes both modified and unmodified files that were
  /// analyzed during the formatting operation.
  final List<String> processedFiles;

  /// List of files that were actually modified.
  ///
  /// Subset of processedFiles that contained Dartdoc comments
  /// requiring reformatting according to the specified criteria.
  final List<String> modifiedFiles;

  /// List of error messages encountered during processing.
  ///
  /// Each error includes the file path and description of the
  /// issue that prevented successful processing.
  final List<String> errors;

  /// The line length that was used for formatting.
  ///
  /// Reflects the actual line length parameter that was applied
  /// during the formatting process.
  final int lineLength;

  /// Whether this was a dry run operation.
  ///
  /// Indicates whether files were actually modified or just
  /// analyzed for potential changes.
  final bool dryRun;
...
///
/// This exception provides structured error information for various
/// failure scenarios that can occur during comment formatting operations.
class DartdocFormatterException implements Exception {
  ...
  ///
  /// Provides clear information about what went wrong and
  /// potential steps for resolution when applicable.
  final String message;

  /// The specific type of error that occurred.
  ///
  /// Categorizes the error for programmatic handling and
  /// appropriate user feedback.
  final DartdocFormatterErrorType type;

  /// Optional underlying cause of this exception.
  ///
  /// Preserves the original exception when this error wraps
  /// another exception for debugging and logging purposes.
  final Object? cause;
...
///
/// This enumeration categorizes the various failure modes to enable
