...
/**
 * Action to fully format Dart files including comments and code with 120 character line length.
 * 
 * This action appears in the context menu when right-clicking .dart files
 * in the editor or project view. It executes the `splendid_cli format`
 * command with a line length of 120 characters on the selected file, which performs:
 * 1. Dartdoc comment formatting (/// and /** */)
 * 2. Regular comment formatting (//)
 ...
 */
class FormatDartFile120Action : AnAction() {
    ...
    /**
     * Executes the format command on the selected Dart file with 120 character line length.
     * 
     * This method runs the CLI command in a background task with progress
     * indication. The format command performs comprehensive formatting including
     * Dartdoc comments, regular comments, and Dart code. After successful
     * completion, the file is refreshed in the IDE to show the changes.
     * 
    ...
    override fun actionPerformed(e: AnActionEvent) {
        ...
        ProgressManager.getInstance().run(object : Task.Backgroundable(
            project,
            "Formatting Dart File (120 chars)",
            false
        ) {
            override fun run(indicator: ProgressIndicator) {
                indicator.text = "Running splendid_cli format on ${file.name}..."
                    ...
                    if (result.isSuccess) {
                        ...
                        if (settings.showSuccessNotifications) {
                            cliExecutor.showNotification(
                                "Format Successful",
                                "${file.name} fully formatted to 120 characters",
