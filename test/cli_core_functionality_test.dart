...
///
/// This test suite focuses on testing the essential CLI functionality that can
/// be validated without external dependencies like Flutter CLI or Mason bricks.
/// It ensures that the core argument parsing, validation, and error handling
/// work correctly.
///
/// Test Categories:
...
///
/// These tests provide confidence that the CLI behaves correctly for the most
/// common user interactions and error scenarios.
void main() {
  group('Splendid CLI Core Functionality', () {
    ...
    group('missing arguments', () {
      ...
      ///
      /// This is the most common scenario when users first try the CLI and
      /// should provide helpful guidance on how to use the tool.
      ...
      /// Tests that CLI returns usage error when create command has no project
      /// name.
      ///
    ...
    group('invalid arguments', () {
      ...
      ///
      /// This ensures that typos in flag names are caught and reported clearly
      /// to users rather than being silently ignored.
      ...
      ///
      /// This ensures that typos in command names are caught and users are
      /// shown the available commands.
    ...
    group('valid arguments', () {
      ...
      /// validation and the CLI proceeds to the next step (which may fail due
      /// to missing Flutter CLI, but that's expected in test environment).
      test('should accept valid project names', () async {
        ...
        for (final String name in validNames) {
          ...
          // (success) or 1 (Flutter CLI missing) depending on environment
          expect(
            exitCode,
            isNot(equals(64)),
    ...
    group('help and usage', () {
      ...
      ///
      /// This ensures that users can get detailed information about how to use
      /// the CLI and what options are available.
      ...
      ///
      /// This ensures that users can get specific help for the create command
      /// including all available options and usage patterns.
    ...
    group('directory handling', () {
      ...
      /// Tests that CLI proceeds when force flag is used with existing
      /// directory.
      ///
      ...
      test('should proceed with force flag for existing directories', () async {
        ...
        // Should not fail due to directory existence (may fail due to Flutter
        // CLI missing) The force flag should override directory check, but may
        // still fail due to missing Flutter CLI
        expect(exitCode, anyOf(equals(0), equals(1)), reason: 'Force flag should override directory check');
      });
    });

    group('exit codes', () {
      /// Tests that CLI returns correct POSIX exit codes for different
      /// scenarios.
      ///
      /// This ensures compatibility with shell scripts and CI/CD systems that
      /// rely on exit code semantics for error handling.
